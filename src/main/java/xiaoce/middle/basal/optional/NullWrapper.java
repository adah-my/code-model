...
/**
 * 工具类，用于简化空指针的探测工作
 * 核心思想：将可能为null的value包装成NullWrapper对象，那么调用nullWrapper.xxx()方法肯定不会发生NullpointerException
 * 核心方法：map()
 * map()方法由NullWrapper对象调用，实际操作的是nullWrapper内部的value，将value映射为指定的值（比如某个字段）
 * 如果value为null，调用empty()方法，返回包装了null的NullWrapper对象
 * 如果value不为null，执行mapper.apply(value)得到结果并调用ofNullable(T newValue)方法，返回包装了newValule的NullWrapperd对象
 * @param <T>
 */
public class NullWrapper<T>
{
     ...
     * 如果value为null，调用empty()方法，返回包装了nulldeNullWrapper对象
